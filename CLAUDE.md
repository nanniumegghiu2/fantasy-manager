...
**`apps/admin` costruito** (Vite+React+TS separato, stesso tema, porta 5174): auth gate con controllo `profiles.is_admin` (l'account dell'utente è stato promosso admin), schermata **Giocatori** (lista/ricerca, form con statistiche fattuali e **Overall calcolato in tempo reale** dal pool via `computeOverallRatings`, override manuale 60-99), schermata **Club** (CRUD nome+stemma), schermata **Sfide giornaliere** (data, tipo sfida, selezione pacchetti club+epoca dal pool esistente). Verificato: build pulita, login screen senza errori console; le schermate autenticate (CRUD reale) vanno verificate dall'utente in prima persona (servono le sue credenziali Google, non replicabili in automatico).
