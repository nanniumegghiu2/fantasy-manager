...
**Stato attuale (2026-07-24)**: punto 1 della roadmap in corso. Fatto: monorepo pnpm, `apps/web` (Vite+React 19+TS+Tailwind v4+Zustand+Framer Motion, tema chiaro/scuro selezionabile e persistito), `packages/shared-types`, `packages/game-engine` (moduli, rating, chemistry, livelli — 14 test vitest verdi), progetto Supabase cloud creato (project ref `krgmwufeshbdlqyivimy`) e **migrazioni + RLS + seed applicati con successo** (8 livelli, 11 moduli, 8 carte tattiche verificati in tabella). `apps/web/.env.local` collegato. Mancante: Auth Google (serve setup manuale Google Cloud Console, prossimo passo), draft/match realtime, UI del draft/match, Edge Functions, `apps/admin`.
