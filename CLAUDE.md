...
**Stato del popolamento (aggiornato ad ogni sessione)**: Serie A 2025/26 — **6 di 20 club fatti**: Inter (campione), Napoli, Roma, Milan, Juventus, Como (152 giocatori totali). Mancano 14 club di Serie A e le altre 4 leghe Big 5 (Premier League, Liga, Bundesliga, Ligue 1) per lo scope completo richiesto dall'utente ("tutte le squadre dei 5 campionati, rose della prima squadra al completo").
...
**Popolamento dati in corso** (sez. 2.3): niente più pipeline Wikidata automatizzata — sostituita da ricerca web mirata per club/stagione (Wikipedia season-specific, non pagine "squad" generiche) + `packages/data-scripts` per il ricalcolo massivo. Fatto: `leagues` (Serie A), 6 club Serie A 2025/26 con rose reali (152 giocatori, statistiche verificate per i big + stime dichiarate per gli altri), colonne `prestige_tier` su `leagues`/`clubs`. Mancano 14 club di Serie A + 4 leghe (Premier League, Liga, Bundesliga, Ligue 1) per completare lo scope "tutte le squadre delle Big 5" richiesto dall'utente — lavoro sostanziale, prosegue su più sessioni.
