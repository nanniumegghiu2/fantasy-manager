...
function Barra({ label, value, tone }: { label: string; value: number; tone: string }) {
  return (
    <span className="flex items-center gap-1.5">
      <span className="text-micro font-bold tracking-wide text-[var(--text-secondary)] uppercase">{label}</span>
      ...
      <span className="text-label font-bold tabular-nums text-[var(--text-secondary)]">{value}</span>
    </span>
  );
}
...
function Riga({ voce, onApri }: { voce: DressingRoomEntry; onApri: () => void }) {
  return (
    <motion.button
      ...
      className={`flex w-full flex-col gap-2 rounded-card border p-3 text-left transition-colors ${
        voce.feud
    ...
    >
      <div className="flex items-center gap-2">
        <p className="min-w-0 flex-1 truncate text-body font-extrabold">{voce.name}</p>
        {voce.feud && (
          <span className="flex items-center gap-1 rounded-control bg-[#ff4d4d]/20 px-1.5 py-0.5 text-label font-extrabold text-[#ff4d4d]">
            <ShieldAlert size={10} /> ROTTURA
          </span>
        )}
        {voce.contractSeasonsLeft <= 1 && (
          <span className="flex items-center gap-1 rounded-control bg-[#ff8a3d]/20 px-1.5 py-0.5 text-label font-extrabold text-[#ff8a3d]">
      ...
      <p className="text-label leading-snug text-[var(--text-secondary)]">▸ {voce.demand}</p>
...
export function SpogliatoioPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {impegni.length > 0 && (
        <section className="rounded-card border border-[var(--surface-border)] bg-[var(--surface-raised)] p-3">
          <p className="flex items-center gap-2 text-micro font-extrabold tracking-widest text-[var(--text-secondary)] uppercase">
            <Handshake size={12} /> Impegni presi · {impegni.length}
          </p>
          <ul className="mt-2 flex flex-col gap-1.5">
            {impegni.map((c) => (
              <li key={c.id} className="flex items-start gap-2 text-label">
